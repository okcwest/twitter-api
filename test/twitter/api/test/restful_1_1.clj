(ns twitter.api.test.restful-1-1
  (:use
   [clojure.test]
   [twitter.test-utils.core]
   [twitter.test creds utils]
   [twitter.callbacks]
   [twitter.api.restful-1-1])
  (:import
   (java.io File)))
...
(deftest test-user
  (let [user-id (get-user-id *user-screen-name*)]
    ...
    (is-200 suggest-users-for-slug-members :params {:slug "sports"})
...
(deftest test-list-members
  (with-list list-id
    ...
    (is (thrown? Exception (show-member :params {:list-id list-id :screen-name *user-screen-name*})))))
...
(deftest test-geo
  (is-200 show-place :params {:place-id "df51dec6f4ee2b2c"})
  (is-200 similar-places :params {:lat "37.7821120598956" :long "-122.400612831116" :name "Twitter HQ"})
  (is-200 reverse-geocode :params {:lat "37.7821120598956" :long "-122.400612831116"})
  (is-200 search-places :params {:query "Los Angeles"}))